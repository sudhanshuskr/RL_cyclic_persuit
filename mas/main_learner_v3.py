...
# q_func = np.load('q_func_PG_1.npy')
...
def get_next_step_BR(ind):
    ...
    for i in range(na):
        ...
        if abs(ay_1) >= 0.98*ylim:
            q_func[q_indices[0],q_indices[1],q_indices[2],q_indices[3],q_indices[4],q_indices[5],i] = 1000000
            continue
        J1 = J_i_PG_int(ax_1,ay_1,angle_0,i,tx_0,ty_0,[cx_1,cy_1],r0,d0,N)
        MIN_QS2 = 1000000
        for k in range(na):
...
for i in range(EPOCH):
    ...
    for episode in tqdm(range(EPISODES), desc= "Episode number : {} of {} | Trajectory cost = {:.2f} ".format(i,EPOCH,traj_cost)):
        ...
        while (collision):
            ...
            if count > 200:
                break
    np.save("q_func_PG_1.npy",q_func)
    np.save("traj_cost_data_PG_1",np.array(traj_cost_list))
