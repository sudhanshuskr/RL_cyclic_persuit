...
def get_next_step_BR(ind):

    ax_0 = ag[ind].x
    ay_0 = ag[ind].y
    tx_0 = ag[ind].target.x
    ty_0 = ag[ind].target.y
    [cx_0,cy_0] = get_centroid(ag)
...
for i in range(EPOCH):
    ...
    for episode in tqdm(range(EPISODES), desc= "Episode number : {} of {} | Trajectory cost = {:.2f} ".format(i,EPOCH,traj_cost)):
        ...
        # a3 = Agent(-8,-8,0,name="3")
        # a4 = Agent(-10,-8,0,name="4")
        # a1 = Agent(random.random()*1- 10,random.random()*1 - 10,name="1")
        # a2 = Agent(random.random()*1 - 8,random.random()*1 - 10,name="2")
        # a3 = Agent(random.random()*1 - 6,random.random()*1 - 8,name="3")
        # a4 = Agent(random.random()*1 - 6,random.random()*1 - 6,name="4")
        # a5 = Agent(random.random()*1 - 10,random.random()*1 - 8,name="5")

        centroid_estimate =np.array([[-10,-10],[-8,-10],[-8,-8],[-10,-8],[-10,-8]])
        # a1.setTarget(a2)
        # a2.setTarget(a3)
        # a3.setTarget(a4)
        # a4.setTarget(a5)
        ...
        for n_agent in range(N):
            ag.append(Agent(temp_center[0] + temp_radius*np.cos(spawn_angle),temp_center[1] + temp_radius*np.sin(spawn_angle)))
        ...
        while (collision):
            count = count + 1
            centroid_temp = centroid_estimate.copy()
            for ind in range(N):
                ax = ag[ind].x
                ay = ag[ind].y
                tx = ag[ind].target.x
                ...
                min_ind = get_next_step_BR(ind)
                q_next_state(ag[ind],min_ind)
                ag_i = ind       
                fi = ag_i
                si = ag_i +1
                if fi == N-1: si = 0
                # centroid_estimate[fi][0] = 0.5*(centroid_temp[fi][0] + centroid_temp[si][0]) + ((1/N)*v*np.cos((min_ind/na)*(2*np.pi)))
                # centroid_estimate[fi][1] = 0.5*(centroid_temp[fi][1] + centroid_temp[si][1]) + ((1/N)*v*np.sin((min_ind/na)*(2*np.pi)))
                # centroid_estimate[fi][0] = cx
                # centroid_estimate[fi][1] = cy
            ...
            [cx,cy] = get_centroid(ag)
            # for p in range(N):
            #     cx_esimate = centroid_estimate[p][0]
            #     cy_esimate = centroid_estimate[p][1]
            #     err = np.linalg.norm([(cx - cx_esimate),(cy - cy_esimate)])
            #     centroid_err_t.append(err)
                
            # centroid_err.append(centroid_err_t)
            ...
            if PLOT == 1:
                ...
                # arr = np.append(arr,centroid_estimate[0][0])
                # arr1 = np.append(arr1,centroid_estimate[0][1])
    ...
    if SAVE == 1:
        np.save("centroid_est_1.npy",centroid_err)
