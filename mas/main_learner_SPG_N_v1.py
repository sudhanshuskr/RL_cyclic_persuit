...
def get_next_step_SPG(ind,cx_0,cy_0):
    ...
    GRAD = grad(l_ax_0,l_ay_0,ax_0,ay_0,ag[ind].theta,i,tx_0,ty_0,[cx_0,cy_0],r0,d0,N)
...
for i in range(EPOCH):
    for episode in tqdm(range(EPISODES), desc= "Episode number : {} of {} | Trajectory cost = {:.2f} ".format(i,EPOCH,traj_cost)):
        ...
        for n_agent in range(N):
            ag.append(Agent(spawn_center[0] + random.uniform(1, 5)+ temp_radius*np.cos(spawn_angle),spawn_center[1] + random.uniform(1, 5)+ temp_radius*np.sin(spawn_angle)))
