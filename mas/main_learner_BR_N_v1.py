...
def get_next_step_BR(ind,cx_0,cy_0):
    ...
    if MIN_QS2>0:
        with open("learn.txt", 'a') as file:
            file.write(f"{ax_0} {ay_0} {tx_0} {ty_0}\n")
    return MIN_QS1_ind
...
def q_update(ind,history,new_indices):    
    ...
    J0 = history[ind]['J0']
    J1 = history[ind]['J1']
    Last = q_func[q_indices[0],q_indices[1],q_indices[2],q_indices[3],q_indices[4],q_indices[5],q_indices[6]]
    New = q_func[new_indices[0],new_indices[1],new_indices[2],new_indices[3],new_indices[4],new_indices[5],new_indices[6]]
    q_func[q_indices[0],q_indices[1],q_indices[2],q_indices[3],q_indices[4],q_indices[5],q_indices[6]]  = ((1 - learning_rate)*Last) + (learning_rate*(J0 + (GAMMA*J1) + ((GAMMA**2)*New) ))
    return
...
# q_func = np.load('q_func_BR_v1.npy')
...
if PLOT ==1 :
    ...
    circle = Circle((-3, -3), radius=2, color='blue', alpha=0.5)  # (0, 0) represents the center of the circle, radius=1 defines the radius of the circle
    ax.add_patch(circle)
    plt.xlim(-15,15)    
    plt.ylim(-15,15)
traj_cost = 0
    ...
    for episode in tqdm(range(EPISODES), desc= "Episode number : {} of {} | Trajectory cost = {:.2f} ".format(i,EPOCH,traj_cost)):
        ...
        while (collision):
            ...
            for ind in range(N):
                ...
                min_ind = get_next_step_BR(ind,cx,cy)
                if episode == EPISODES-1:
                    temp_search = q_func[temp_indices[0],temp_indices[1],temp_indices[2],temp_indices[3],temp_indices[4],temp_indices[5],:]
                    min_ind = np.argmin(temp_search)
                ...
                if collision == 0:
                    ...
                    break
                ax_1 = ax + v*np.cos((min_ind/na)*(2*np.pi))
                ay_1 = ay + v*np.sin((min_ind/na)*(2*np.pi))
                tx_1 = tx
                ty_1 = ty
                cx_1 = cx + (v/N)*np.cos((min_ind/na)*(2*np.pi))
                cy_1 = cy + (v/N)*np.sin((min_ind/na)*(2*np.pi))
                temp_a = Agent(ag[ind].x , ag[ind].y)
                temp_indices = get_q_index(ax_1,ay_1,tx_1,ty_1,cx_1,cy_1)
                temp_search = q_func[temp_indices[0],temp_indices[1],temp_indices[2],temp_indices[3],temp_indices[4],temp_indices[5],:]
                min_ind = np.argmin(temp_search)
                temp_indices.append(min_ind)
            ...
            for ind in range(N):
                q_next_state(ag[ind],controls[ind])
            [cx,cy] = get_centroid(ag)
